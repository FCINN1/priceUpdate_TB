...
async function main() {
  try {
    ...
    const TB_LIST = await playerSearch([206], 0); // playerSearch(시즌넘버, 최소오버롤)
    let TB_RESULTS = await playerPriceValue(TB_LIST, [1, 2, 3, 4, 5, 6, 7, 8]); // playerPriceValue(데이터 , 강화등급)
    await saveToDB(TB_RESULTS);
